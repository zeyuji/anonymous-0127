...
for MODEL in "${MODELS[@]}"; do
    for JUDGE_MODE in "${JUDGE_MODES[@]}"; do
        for MAX_SCORE in "${MAX_SCORES[@]}"; do
            echo "==============================================="
            echo "FACT: MODEL: ${MODEL}, JUDGE_MODE: ${JUDGE_MODE}, MAX_SCORE: ${MAX_SCORE}"
            echo "==============================================="

            python -m Src.judge.judge_model \
                --dataset_config ./Dataset_Configs/trivia_qa.yaml \
                --results_dir "./LLM_Response/New_7B_Ensemble/PEERREVIEW_AVERAGE/FACT/${JUDGE_MODE}_${MAX_SCORE}" \
