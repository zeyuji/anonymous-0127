...
def run_agent_forest(
    ...
    # [n_models, n_samples]
    test_generations_for_agent_forest = test_generations_for_agent_forest.transpose()
    # [n_samples, n_models]

    dataset_texts = []
    agent_forest_logs = []
    bleu_scores = []
    n_samples = len(test_generations_for_agent_forest)

    for i in range(n_samples):
        index, scores = most_similar_response(test_generations_for_agent_forest[i])
        agent_forest_logs.append(index)
        bleu_scores.append(scores)
        dataset_texts.append(test_generations_for_agent_forest[i][index])
    results_lines = [
        {
